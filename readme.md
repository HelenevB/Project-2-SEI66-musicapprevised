...
* Run the “npm install” command to download all the required dependencies
* Set up a “.env” file: 
    * configure to a port (line 15 server.js file)
    * configure to your local MongoDB database (line 68 server.js file)
    * configure your secret (line 42 server.js file)  
 * Run the “npm test” command to start the application 
...
To explore the features available to the general user, please use the following log-in credentials: 
...
*	Functionality requirements: 
	...
	* a user must be able to create , edit, review, and delete a resource 
* This was a paired project alongside [Ellie Eadington](https://github.com/ellieeadington) 
* Timeframe: 7 Days <br>
### **Technologies Used**
---
* Express
...
<img src="public/ReadME/artistindex.png" width=45%> <img src="public/ReadME/artistdetail.png" width=45%>
...
---
For this project, we decided to build the application around a subject we could both get stuck into; before landing on any ideas, we discussed our interests. We established through this that we both had a passion for music and enjoyed going to gigs, so we agreed a music events app would be the way forward. 
...
The features I was responsible for in the build process were: 
...
* If “Artist” were selected, additional fields would be displayed to complete before form submission 
...
2. Include multipart/form-data as part of form submission to allow files to be submitted
...
These functions could then be exported and called upon anytime image upload was required. 
...
* The artist-user profile update functionality is not complete, so artists  are currently unable to update their bio  
* Success or failure messages after form submissions still need to be completed; the user is not getting notified  if sign-up or sign-in is successful  
...
* Sign-up/ Sign-in is not displayed clearly on the navigation bar. It is currently as a dropdown, which on review, is not very practical<br>
...
* Split workload more effectively to ensure that all team members are working on MVP features before starting on enhanced features such as filtering.  
